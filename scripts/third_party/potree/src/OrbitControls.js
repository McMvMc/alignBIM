...
Potree.OrbitControls = function ( object, domElement ) {
	...
	function dblclick( event ){
		...
		var dest = ray.origin.clone();
		dest = dest.add(ray.direction.multiplyScalar(1000));
		var material = new THREE.LineBasicMaterial({ color: 0xff0000 }); 
		var geometry = new THREE.Geometry(); 
		geometry.vertices.push( ray.origin, dest); 
		var line = new THREE.Line( geometry, material ); 
		scene.add( line );       

		// var raycaster = new THREE.Raycaster();
		// raycaster.setFromCamera( mouse, camera );
		// var intersects = raycaster.intersectObjects( Array.from(BIM.children[0].geometry.attributes.position.array) );
